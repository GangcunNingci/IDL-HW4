...
class ScaledDotProductAttention:
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        self.V = V
        self.mask = mask
        self.dk = self.Q.shape[-1]
        # Calculate attention scores: (N, ..., H, L, S)
        # (N, ..., H, L, E) @ (N, ..., H, E, S) -> (N, ..., H, L, S)
        scaled_dot_product = np.matmul(self.Q, np.swapaxes(self.K, -1, -2)) / np.sqrt(self.dk)
        
        # Apply mask before softmax if provided
        # If mask is not None, add -self.eps to the attention scores for positions to ignore
        if mask is not None:
            scaled_dot_product = scaled_dot_product = scaled_dot_product + (-self.eps * mask.astype(scaled_dot_product.dtype))
        ...
        output = np.matmul(self.attention_scores, self.V)
