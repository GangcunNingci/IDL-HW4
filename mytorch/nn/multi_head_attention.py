...
class MultiHeadAttention:
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        key_mask = np.broadcast_to(key_padding_mask.astype(bool)[:, None, None, :], (self.N, self.num_heads, self.L, self.S))
        
        # Expand attn_mask to (1, 1, L, S) and broadcast to (N, H, L, S)
        attention_mask = np.broadcast_to(attn_mask.astype(bool)[None, None, :, :], (self.N, self.num_heads, self.L, self.S))
